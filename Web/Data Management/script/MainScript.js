...
function handle_geolocation_query(position) {
  localStorage.setItem("pos_latitude", position.coords.latitude);
  localStorage.setItem("pos_longitude", position.coords.longitude);
  onPositionReady();
}
...
var main = function() {
  ...
  var newTableRow = $("<tr>").append(
	...
	$("<td>"),
    $("<td>"),
    $("<td><span>" + localStorage.getItem("pos_latitude") + "</span></td>"),
    $("<td><span>" + localStorage.getItem("pos_longitude") + "</span></td>"),
    $("<td>")
  );
  // Add a row
  ...
  $("#serializeTable").click(function() {
    ...
    var newFile = new Blob([JSON.stringify(dataFromTA)], {
    ...
    try {
      ...
      for (var i = 0; i < tableData.length; i++) {
        $(".table tbody").append(
          $("<tr>").append(
            $("<th>").html(i + 1),
			$("<td>").html(tableData[i].id),
            $("<td>").html(tableData[i].name),
            $("<td>").html(tableData[i].latitude),
            $("<td>").html(tableData[i].longitude),
            $("<td>").html(tableData[i].description)
          )
        );
      }
      updatePopover();
